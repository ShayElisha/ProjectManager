...
export function riskPriority(score: number): "low" | "medium" | "high" {
...
function addSuggestion(
  ...
  keySet: Set<string>,
  item: Omit<RiskSuggestion, "score"> & { probability: RiskLevel; impact: RiskLevel },
): void {
...
export function suggestRisksFromProject(input: SuggestRisksInput): RiskSuggestion[] {
  ...
  if (input.lateTaskCount >= 3 && !keySet.has("late:aggregate")) {
    const prob: RiskLevel = input.criticalLateCount > 0 ? "high" : "medium";
    const imp: RiskLevel = input.criticalLateCount > 0 ? "high" : "medium";
    ...
    for (const t of sortedLate.slice(0, 5)) {
      const key = `late:${t.id}`;
      const prob: RiskLevel = t.isCritical ? "high" : "medium";
      const imp: RiskLevel = t.isCritical ? "high" : "medium";
      addSuggestion(suggestions, keySet, {
        key,
        title: `איחור: ${t.name}`,
  ...
  if (criticalOpen.length > 0 && !keySet.has("schedule:critical-path")) {
    const prob: RiskLevel = criticalOpen.length >= 3 ? "high" : "medium";
    const imp: RiskLevel = "high";
  ...
  for (const t of dueSoon.slice(0, 3)) {
    ...
    const prob: RiskLevel = daysLeft <= 2 ? "high" : "medium";
    const imp: RiskLevel = t.isCritical ? "high" : "medium";
    addSuggestion(suggestions, keySet, {
      key: `due:${t.id}`,
  ...
  if (input.forecastDelayDays != null && input.forecastDelayDays >= 5 && !keySet.has("schedule:forecast")) {
    const prob: RiskLevel = input.forecastDelayDays >= 14 ? "high" : "medium";
    const imp: RiskLevel = "high";
    addSuggestion(suggestions, keySet, {
  ...
  if (
    input.baselineLateFinishCount != null &&
    input.baselineLateFinishCount >= 2 &&
    !keySet.has("baseline:drift")
  ) {
    const prob: RiskLevel = input.baselineLateFinishCount >= 5 ? "high" : "medium";
    const imp: RiskLevel = "high";
    addSuggestion(suggestions, keySet, {
      key: "baseline:drift",
      title: "סטייה מ-Baseline",
      description: `${input.baselineLateFinishCount} משימות עם סיום מאוחר מול baseline${
        input.baselineAvgEndVarianceDays != null
  ...
  if (input.projectHealth && input.projectHealth !== "on_track") {
    ...
    if (!keySet.has(key)) {
      const prob: RiskLevel = input.projectHealth === "critical" ? "high" : "medium";
      const imp: RiskLevel = "high";
      addSuggestion(suggestions, keySet, {
        key,
        title:
          input.projectHealth === "critical"
            ? "בריאות פרויקט: קריטי"
            : "בריאות פרויקט: בסיכון",
        description:
          "שילוב של מדדי EVM (SPI/CPI), משימות באיחור וחריגות תקציב מצביע על מצב בסיכון.",
        category: "schedule",
        probability: prob,
        impact: imp,
  ...
  if (input.evm) {
    if (input.evm.spi < 0.9 && !keySet.has("evm:spi")) {
      const prob: RiskLevel = input.evm.spi < 0.8 ? "high" : "medium";
      const imp: RiskLevel = "high";
      addSuggestion(suggestions, keySet, {
        key: "evm:spi",
    ...
    if (input.evm.cpi < 0.9 && !keySet.has("evm:cpi")) {
      const prob: RiskLevel = input.evm.cpi < 0.8 ? "high" : "medium";
      const imp: RiskLevel = "high";
      addSuggestion(suggestions, keySet, {
        key: "evm:cpi",
  ...
  if (
    input.unassignedCriticalCount != null &&
