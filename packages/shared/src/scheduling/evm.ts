...
export function plannedPctForCashMonth(cashMonth: string, asOfDate: string): number {
  const asOfMonth = asOfDate.slice(0, 7);
  if (asOfMonth > cashMonth) return 1;
  if (asOfMonth < cashMonth) return 0;
...
function taskPlannedPct(t: Task, asOf: number): number {
  ...
  if (asOf > start) return (asOf - start) / span;
  return 0;
}

function taskEarnedValue(t: Task, taskBudget: number): number {
  if (t.earnedValue != null && t.earnedValue >= 0) return t.earnedValue;
  return taskBudget * (t.percentComplete / 100);
}

function lineEarnedValue(line: BudgetLineItem): number {
...
export function calculateEVM(
  ...
  const bac =
    projectBudget > 0 ? projectBudget : taskPlannedSum + linePlannedSum;
